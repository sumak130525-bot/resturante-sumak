...
export async function GET(request: NextRequest) {
  ...
  // 3. Costo mercadería: sumar total_cost de inventory_movements tipo 'purchase'
  // (total_cost es el precio real de la factura, no quantity * precio unitario)
  ...
  const totalMercaderia = (movements ?? []).reduce(
    (sum: number, m: { total_cost: number }) => sum + Number(m.total_cost ?? 0), 0
  )
  ...
  for (const bucket of buckets) {
    ...
    // Sumar total_cost directamente (precio real de factura)
    const mMercaderia = (mMovements.data ?? []).reduce(
      (s: number, m: { total_cost: number }) => s + Number(m.total_cost ?? 0), 0
    )
